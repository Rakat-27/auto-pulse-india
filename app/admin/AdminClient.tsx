...
const errorText = (value: unknown) => value instanceof Error ? value.message.replace("Firebase: ", "") : "Something went wrong.";
...
function Products({ items }: { items: Item[] }) {
  ...
  async function upload(file?: File) {
    if (!file) return; if (!file.type.startsWith("image/") || file.size > 8 * 1024 * 1024) { setError("Choose an image smaller than 8 MB."); return; } setBusy(true);
...
function Setup({ user }: { user: User }) {
  const checks: [string, boolean, string][] = [["Firebase project", Boolean(process.env.NEXT_PUBLIC_FIREBASE_PROJECT_ID), "Firestore and Authentication"], ["Cloudinary cloud", Boolean(cloudinaryConfig.cloudName), "Image delivery account"], ["Unsigned upload preset", Boolean(cloudinaryConfig.uploadPreset), "Browser image uploads"], ["Fixed admin account", true, user.email || "Not signed in"]];
  return <div className="grid gap-6 xl:grid-cols-2"><Panel title="Service status"><div className="space-y-3">{checks.map(([label, ok, detail]) => <div key={label} className="flex items-center gap-3 rounded-xl border p-4"><div className={cx("grid h-9 w-9 place-items-center rounded-full", ok ? "bg-emerald-50 text-emerald-600" : "bg-amber-50 text-amber-600")}>{ok ? <FiCheckCircle /> : <FiSettings />}</div><div><p className="text-sm font-bold">{label}</p><p className="break-all text-xs text-slate-500">{detail}</p></div><span className={cx("ml-auto text-[10px] font-bold uppercase", ok ? "text-emerald-600" : "text-amber-600")}>{ok ? "Ready" : "Needs setup"}</span></div>)}</div></Panel><Panel title="Finish setup"><ol className="list-decimal space-y-4 pl-5 text-sm leading-6 text-slate-600"><li>Enable Email/Password in Firebase Authentication.</li><li>Create the single Firebase Auth email account shown above, using the same password as <b>ADMIN_PASSWORD</b>.</li><li>Deploy the included <b>firestore.rules</b>.</li><li>Create an unsigned Cloudinary preset restricted to images.</li><li>Add the Cloudinary values from <b>.env.example</b> to <b>.env</b> and restart Next.js.</li></ol></Panel></div>;
}
